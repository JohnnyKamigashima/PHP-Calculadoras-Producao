    ...
    var upperPrimeiro = docRef.channels[primeiroCanal].name.toUpperCase(),
        ...
        if(upperPrimeiro.indexOf("SUBSTRATO") == -1 && upperPrimeiro.indexOf("TECH") == -1 && upperPrimeiro.indexOf("DIE") == -1 &&upperPrimeiro.indexOf("REGUA") == -1 && upperPrimeiro.indexOf("COLOR") == -1 &&upperPrimeiro.indexOf("VARNISH") == -1 ){
            ...
            if(primeiroCanal!=0 && primeiroRef.visible == true){
             ...
             for(var segundoCanal = primeiroCanal+1; segundoCanal < Ncanais; segundoCanal++){

                var upperSegundo = docRef.channels[segundoCanal].name.toUpperCase(),
                segundoRef = docRef.channels[segundoCanal];

                if(upperSegundo.indexOf("SUBSTRATO") == -1 && upperSegundo.indexOf("TECH") == -1 && upperSegundo.indexOf("DIE") == -1 &&upperSegundo.indexOf("REGUA") == -1 && upperSegundo.indexOf("COLOR") == -1 &&upperSegundo.indexOf("VARNISH") == -1 ){

                    segundoRef.visible = true;
                    app.refresh();
                    wait(1000);
                    segundoRef.visible = false;
                }
            }
     
        if(primeiroCanal+1 < Ncanais){
            docRef.channels[primeiroCanal+1].visible = true;

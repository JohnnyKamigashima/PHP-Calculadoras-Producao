...
function readAndCleanupReportFile($target_file)
{
 ...
 function limpaXFDFTexto($texto) {
     ...
     $texto = preg_replace('/(\n)/Umi', ' ', $texto);
     $texto = preg_replace('/(<\/.*?>)/mi', '$1
     ', $texto);
     $texto = preg_replace('/(<(span.*font-weight:bold.*|strong)>)(.*)(<\/(span|strong).*>)/Umi', '
     [strong]$3[/strong]
     ', $texto);
 $texto = preg_replace('/(<(span.*font-weight:italic.*|em)>)(.*)(<\/(span|em).*>)/Umi', '
 [em]$3[/em]
 ',$texto);
    $texto= preg_replace('/\[(strong|em)\]/imU','
    [$1]',$texto); 
     $texto = preg_replace('/(<.*>|&#xD;)/mU', ' ',$texto);

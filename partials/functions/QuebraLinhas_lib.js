module.exports = function quebraLinhas(id) {
    ...
    this.carregaPalavrasQuebra("Abreviacoes.txt")
        .then(naoQuebrarPalavras => {
            let abreviacoesRegex = new RegExp(
                `(((?<!(${naoQuebrarPalavras}))\\.)|(;))`,
                'gim'
            );
    ...
    this.carregaPalavrasQuebra("QuebraLinhas.txt")
        .then(textoQuebraLinhas => {
            let estiloRegex = RegExp(
                '(<(strong|em)>.*)(<br>)(.*<\\/(strong|em)>)',
                'gim'
            );
            let textoQuebraLinhaRegex = new RegExp(
                `(${textoQuebraLinhas})`,
                'gim'
            );
            textoQuebraPontos = textoQuebraPontos.replace(
                textoQuebraLinhaRegex,
                '<br>$1'
            );
            textoQuebraPontos = textoQuebraPontos.replace(
                /(<\/(strong|em)>)/gim,

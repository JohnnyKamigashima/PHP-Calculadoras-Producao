function mm2px(resolucao, mm) {
  return isNaN(Math.ceil((resolucao.val() / 25.4) * mm.val()))
    ? 0
    : Math.ceil((resolucao.val() / 25.4) * mm.val());
}

function px2mm(resolucao, px) {
  return isNaN(px.val() / (resolucao.val() / 25.4))
    ? 0
...
function escalaPlano(medA, medR) {
  return isNaN((medR.val() * 100) / medA.val())
    ? 0
    : ((medR.val() * 100) / medA.val()).toFixed(2);
}

function distorC(medA, medR, rep) {
  return isNaN((medR.val() * 100) / (medA.val() * rep))
    ? 0
    : ((medR.val() * 100) / (medA.val() * rep)).toFixed(3);
}
...
function verificaoctagono(array, valor) {
